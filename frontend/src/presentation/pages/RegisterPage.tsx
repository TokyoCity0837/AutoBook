...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            await authRepository.register(username, password, displayName)
            navigate('/login')
        } catch (err: any) {
            ...
            <div className="authCard">
                ...
                <form className="authForm" onSubmit={handleSubmit}>
                    <div className="authField">
                        ...
                        <input className="authInput" type="text" placeholder="your_nickname" value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" required />
                    </div>
                    <div className="authField">
                        <label className="authLabel">Display name</label>
                        <input className="authInput" type="text" placeholder="display name" value={displayName} onChange={e => setDisplayName(e.target.value)} required />
                    </div>
                    <div className="authField">
                        <label className="authLabel">Email</label>
                        <input className="authInput" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" required />
                    </div>
                    <div className="authField">
                        <label className="authLabel">Password</label>
                        <div className="authInputWrap">
                            <input className="authInput" type={showPass ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={e => setPassword(e.target.value)} autoComplete="new-password" required />
                    ...
                    <div className="authField">
                        ...
                        <div className="authInputWrap">
                            <input className={`authInput${!passwordMatch ? ' inputError' : ''}`} type={showPass ? 'text' : 'password'} placeholder="Repeat password" value={confirm} onChange={e => setConfirm(e.target.value)} autoComplete="new-password" required />
                        </div>
                    ...
                    <label className="authCheckbox">
                        ...
                        <span className="checkText">I agree to the <Link to="/terms" className="authSwitchLink">Terms</Link> and <Link to="/privacy" className="authSwitchLink">Privacy Policy</Link></span>
                    </label>
                    <button className={`authBtn${loading ? ' loading' : ''}`} type="submit" disabled={loading || !agree || !passwordMatch}>
                ...
                <div className="authSocial">
                    <button className="authSocialBtn" type="button"><GoogleIcon /></button>
                    <button className="authSocialBtn" type="button"><FacebookIcon /></button>
                </div>
                <p className="authSwitch">Already have an account? <Link to="/login" className="authSwitchLink">Sign in</Link></p>
            </div>
        </div>
    )
}
